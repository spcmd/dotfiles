  ...
  data[calendarbutton].first_day_widget = first_day_of_current_month
	...
	for i=1,42 do
		if i< first_day_of_current_month then
			data[calendarbutton].month_days_cells[i].widget:set_text(util.escape("")) -- customized, original was: util.escape("--")
			apply_style(data[calendarbutton].month_days_cells[i].widget, data[calendarbutton].days_of_month_widget_style)
	...
	local text_info = nil
            if data[calendarbutton].default_info == nil then
						  text_info = os.date("<span font='26'>   </span><span weight='bold'>\n\n   Today:</span>\n\n%B %d, %Y\n\n%A") -- customized
            end
            if type(data[calendarbutton].default_info) == "string" then
              text_info = data[calendarbutton].default_info
            end
...
local function generate_calendar_box(calendarbutton)
	...
	data[calendarbutton].prev_month.widget:set_text(util.escape("     ")) -- customized: added extra spaces + font awesome symbol
	...
	data[calendarbutton].next_month.widget:set_text(util.escape("             ")) -- customized: added extra spaces + font awesome symbol
	...
	data[calendarbutton].corner.widget:set_text(util.escape('')) -- customized, original was: util.escape('/')
